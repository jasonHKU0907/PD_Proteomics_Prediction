...
dpath = '/Volumes/JasonWork/Projects/PD_Proteomics/Neurology_Revision/Results/ProPanelSelection/XGB_PDRP/'
output_img_dir = dpath + 's3_AccAUC_TotalGain.png'
pro_imp_df = pd.read_csv(dpath + 's2_FeaImportance.csv', usecols = ['Pro_code', 'TotalGain_cv'])
pro_imp_df.rename(columns = {'TotalGain_cv': 'Pro_imp'}, inplace = True)
pro_auc_df = pd.read_csv(dpath + 's3_AccAUC_TotalGain.csv', usecols = ['Pro_code', 'AUC_mean', 'AUC_std', 'p_delong'])
f_dict = pd.read_csv('/Volumes/JasonWork/Projects/PD_Proteomics/Revision_R2/Results/Plots/CovariateInfo.csv')
...
mydf['Predictor'].replace(['NEFL', 'EDA2R', 'ITGAV', 'IL13RA1', 'BAG3', 'SCG2', 'PTPRN2', 'GFAP',
                           'LXN', 'TNXB', 'WARS', 'CLEC10A', 'NCS1', 'NTF4', 'NCAM1'],
                          ['Plasma NEFL', 'Plasma EDA2R', 'Plasma ITGAV', 'Plasma IL13RA1', 'Plasma BAG3',
                           'Plasma SCG2', 'Plasma PTPRN2', 'Plasma GFAP', 'Plasma LXN', 'Plasma TNXB',
                           'Plasma WARS', 'Plasma CLEC10A', 'Plasma NCS1', 'Plasma NTF4', 'Plasma NCAM1'], inplace = True)
