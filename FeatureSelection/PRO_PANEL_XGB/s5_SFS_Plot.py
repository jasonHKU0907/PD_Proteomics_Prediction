...
dpath = '/Volumes/JasonWork/Projects/PD_Proteomics/Neurology_Revision/Results/ProPanelSelection/XGB_ProPANEL/'
output_img_dir = dpath + 's5_AccAUC_TotalGain.png'
pro_imp_df = pd.read_csv(dpath + 's4_PROImportance.csv', usecols = ['Pro_code', 'TotalGain_cv'])
pro_imp_df.rename(columns = {'TotalGain_cv': 'Pro_imp'}, inplace = True)
pro_auc_df = pd.read_csv(dpath + 's5_AccAUC_TotalGain.csv', usecols = ['Pro_code', 'AUC_mean', 'AUC_std', 'p_delong'])
...
ax.set_xticklabels(mydf['Pro_code'], rotation=45, fontsize=14, horizontalalignment='right')
