...
pro_eid_df = pd.read_csv('/Volumes/JasonWork/Projects/PD_Proteomics/Data/Proteomics/ProteomicsData.csv', usecols = ['eid'])
region_code_df = pd.read_csv(dpath1 + 'Eid_info_data.csv', usecols = ['eid', 'Region_code'])
info_df = pd.merge(pro_eid_df, region_code_df, how = 'inner', on = 'eid')
nb_ind = len(info_df)
...
cov_df7 = pd.read_csv('/Volumes/JasonWork/Projects/PD_Proteomics/Data/Covariates/Raw/AdditionalCov.csv')
...
mydf = pd.merge(mydf, cov_df7, how = 'left', on = ['eid'])
...
continuous_f_lst = ['Age', 'TDI',
                    ...
                    'PM_HGS', 'PM_PR', 'LS_AT', 'PRS',
                    'PM_FVC', 'PM_FEV1', 'PM_FEV1_FVC', 'BF_EC_TE', 'BF_BC_MRV', 'PM_LFP',
...
mydf_out.reset_index(inplace = True, drop = True)


mydf_out.rename(columns = {'Age':'DM_AGE', 'Gender':'DM_GENDER', 'Ethnicity':'DM_ETH', 'TDI':'DM_TDI', 'Education':'DM_EDUC',
...
mydf_out = mydf_out[['eid', 'Region_code', 'PRS',
                     'DM_AGE', 'DM_GENDER', 'DM_ETH', 'DM_TDI', 'DM_EDUC',
                     'LS_ALC', 'LS_SMK', 'LS_PA', 'LS_DIET', 'LS_SOC', 'LS_SLP', 'LS_AT', 'LS_WP', 'LS_MG', 'LS_DN',
...
mydf_out.to_csv('/Volumes/JasonWork/Projects/PD_Proteomics/Data/Covariates/PANEL_raw.csv', index = False)
