...
outfile = outpath + 'UKB_ALL/PHENO/AccAUC_TotalGain.csv'

tmp_f_df = pd.read_csv(outpath + 'UKB_ALL/PHENO/FeaImportance.csv')
...
cov_df = pd.read_csv(dpath + 'Covariates/PANEL_raw.csv', usecols = ['eid', 'Region_code'] + tmp_f_lst)
cov_dict = pd.read_csv(dpath + 'Covariates/CovariateInfo.csv', usecols = ['Covariate_code', 'Predictor'])
...
myout_df = pd.merge(AUC_df, cov_dict, how = 'left', left_on='Pro_code', right_on='Covariate_code')
