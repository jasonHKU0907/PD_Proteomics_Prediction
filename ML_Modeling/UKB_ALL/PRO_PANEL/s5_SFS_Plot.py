...
output_img_dir = dpath + 'UKB_ALL/PRO_PANEL/s5_AccAUC_TotalGain.png'
pro_imp_df = pd.read_csv(dpath + 'UKB_ALL/PRO_PANEL/s4_PROImportance.csv', usecols = ['Pro_code', 'TotalGain_cv'])
pro_imp_df.rename(columns = {'TotalGain_cv': 'Pro_imp'}, inplace = True)
pro_auc_df = pd.read_csv(dpath + 'UKB_ALL/PRO_PANEL/s5_AccAUC_TotalGain.csv', usecols = ['Pro_code', 'AUC_mean', 'AUC_std', 'p_delong'])
