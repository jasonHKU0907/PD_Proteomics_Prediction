...
# Calculate the AUC based on predicted probabilities derived on modeling itself
...
#mydf = pd.read_csv(dpath + 'UKB_OVER10YEARS/FULL/FULL_PredProbs.csv')
#outfile = dpath + 'UKB_OVER10YEARS/FULL/FULL_SummaryStatistics.csv'
fold_id_lst = [i for i in range(10)]

cutoff_list = [i*0.0001 for i in range(0, 200)]
Youden_cv = [Youden_index(mydf.target_y, threshold(mydf.y_pred_probs, i*0.0001)) for i in range(0, 200)]
opt_ct_idx = Youden_cv.index(np.max(Youden_cv))
opt_ct = 0.0001*(opt_ct_idx+1)
