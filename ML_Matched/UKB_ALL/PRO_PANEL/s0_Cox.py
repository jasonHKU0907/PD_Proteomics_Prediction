...
outpath = '/Volumes/JasonWork/Projects/PD_Proteomics/Results/'
...
m_f_lst = ['age', 'sex', 'ethn', 'educ', 'tdi', 'smk', 'alc', 'reg_pa', 'sbp', 'bmi']
...
for pro_f in pro_f_lst:
    ...
    cph = CoxPHFitter()
    my_formula = "age + sex + C(ethn) + educ + tdi + smk + alc + reg_pa + sbp + bmi + target_pro"
    try:
...
myout_df.to_csv(outpath + 'UKB_ALL/PRO_PANEL/Cox.csv', index = False)
