...
outputfile = outpath + 'UKB_ALL_Matched/FULL/FULL_PredProbs.csv'

m1_f_lst = ['DM_EDUC', 'RF_TBI', 'PM_HGS', 'BF_EC_IGF1']
m2_f_lst = ['ITGAV', 'IL13RA1', 'SCG2', 'NEFL']

target_df = pd.read_csv(dpath + 'TargetOutcomes/PD/PD_outcomes.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
cov_df = pd.read_csv(dpath + 'Covariates/PANEL_raw.csv', usecols = ['eid', 'Region_code'] + m1_f_lst)
pro_df = pd.read_csv(dpath + 'Proteomics/ProteomicsData.csv', usecols = ['eid'] + m2_f_lst)
...
pred_df = get_pred_probs(m1_f_lst+m2_f_lst, mydf, fold_id_lst, my_params, 'probs')
